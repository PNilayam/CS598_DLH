...
# Below functions declared by Rudra Bhaskar ###
...
def clean_notes(text):
    ...
    clean_text = regex_pattern.sub(replace_char, clean_text)

    return clean_text


def read_notes_table(mimic3_path):
    pats = dataframe_from_csv(os.path.join(mimic3_path, 'NOTEEVENTS.csv'))
    ...
    pats.drop(columns=['ISERROR'])
    pats['TEXT'] = pats['TEXT'].apply(clean_notes)
    return pats

# END OF Below functions declared by RUdra Bhaskar ###
...
def add_age_to_icustays(stays):
    stays['AGE'] = (stays.INTIME - stays.DOB).apply(lambda s: s /
                                                    np.timedelta64(1, 's')) / 60./60/24/365
...
def filter_notes_on_stays(notes, stays):
    return notes.merge(stays[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID']].drop_duplicates(), how='inner',
                           left_on=['SUBJECT_ID', 'HADM_ID'], right_on=['SUBJECT_ID', 'HADM_ID'])
...
#Added By BR
def break_up_notes_by_subject(notes, output_path, subjects=None):
    subjects = notes.SUBJECT_ID.unique() if subjects is None else subjects
    nb_subjects = subjects.shape[0]
    for subject_id in tqdm(subjects, total=nb_subjects, desc='Breaking up notes  by subjects'):
        dn = os.path.join(output_path, str(subject_id))
        try:
            os.makedirs(dn)
        except:
        ...
        notes[notes.SUBJECT_ID == subject_id].sort_values(by=['ICUSTAY_ID'])\
                                                     .to_csv(os.path.join(dn, 'notes.csv'), index=False)
